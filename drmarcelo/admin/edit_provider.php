...
if(isset($_REQUEST['sub']))
{
  if($_FILES['image']['name']!='')
  {
	...
	$ongoing_query = "update `dentalmarcelo_provider` set `title`=:title,bio=:bio,cat_id=:cat_id,page=:page,image=:image where id=:id";
      ...
      {
        $ongoing_query = "update `dentalmarcelo_provider` set `title`=:title,bio=:bio,cat_id=:cat_id,page=:page where id=:id";
...
?>
<?php
 $query_device = "select * from `dentalmarcelo_provider` where id=:id order by id desc";
...
<textarea name="bio"><?php echo $provider->bio;?></textarea> 
  ...
  <p><label><span>*</span>Designation :</label>
  	<select name="cat_id">
  	<?php
  	 $query_device = "select * from `dentalmarcelo_category`";
