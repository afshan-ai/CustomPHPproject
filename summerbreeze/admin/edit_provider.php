...
if(isset($_REQUEST['sub']))
{
  if($_FILES['image']['name']!='')
  {
	...
	$ongoing_query = "update `dentalsb_provider` set `title`=:title,bio=:bio,cat_id=:cat_id,page=:page,image=:image where id=:id";
      ...
      {
        $ongoing_query = "update `dentalsb_provider` set `title`=:title,bio=:bio,cat_id=:cat_id,page=:page where id=:id";
...
?>
<?php
 $query_device = "select * from `dentalsb_provider` where id=:id order by id desc";
